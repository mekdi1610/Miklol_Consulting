...
header( 'Content-Type: text/html; charset=utf-8' );
    ...
    include_once('../php/Stories.php');
    include 'side-menu.php';

    $StoryObj = new Story();

    $accept = $StoryObj->viewStory();

    
    if (isset($_POST['Add'])) {     
        $title = $_POST["title"];  
        $description = $_POST["description"]; 
       $saved= $StoryObj->addStory($title,$description);
        ...
        exit();
		   ...
		   $StoryObj->updateStory($ID, $title, $description);
		  
        }
        else if(isset($_POST['Remove'])){
			$ID = $_POST["idRemove"]; 
			$StoryObj->removeStory($ID);
        }
        ...
        <div class="container-fluid">
            <div class="panel panel-headline">
                        ...
                        <div class="panel-heading">
                            <div class=" navbar-btn-right">
                                <a class="btn btn-success" href="#add" data-toggle="modal" name="Set"
                                    title="Add Story" onclick="openAdd()" title="Upgrade to Pro"><i
                                        class="fa fa-plus"></i> <span>New Certificate</span></a>
                            </div>
                        </div>
                    </div>
                </div>
				...
				<table class="table table-hover">
						<thead>
							<tr><th>#</th><th>Title</th><th>Description</th>
						</thead>
						<tbody>
                        <?php
                                            $StoryID = $accept[0];
                ...
                for ($i = 0; $i < sizeof($StoryID); $i++) {
					echo '<tr>';
							echo "<td>".$StoryID[$i]."</td>";
                            echo '<td>'.$title[$i].'</td>';
                            echo '<td>'.$description[$i].'</td>';
							echo '<td><a href="#modal2" data-toggle="modal" name="Set" data-id="'.$StoryID[$i].'" data-title="'.$title[$i].'" data-description="'.$description[$i].'" class="modal-trigger"><i class="fa fa-edit" onclick="openModal2()"></i></a></td>';
							echo '<td><a href="#modal3" data-toggle="modal" name="Set" data-id="'.$StoryID[$i].'" data-title="'.$title[$i].'" class="modal-trigger"><i class="fa fa-remove" onclick="openModal3()"></i></a></td>';
                           echo '</tr>';
                 } 
				 ?>
				 </tbody>
                ...
                <div class="panel">
                    ...
                    <div class="panel-body">
                        <form method="post" enctype="multipart/form-data" action="stories.php">
                            ...
                            <input type="text" class="form-control" name="title" placeholder="Title" name="Title"><br>

                            <textarea class="form-control" name="description" placeholder="Description" name="Description"
                                rows="4"></textarea><br>
...
<div class="modal" id="modal2" tabindex="-1" role="dialog" aria-hidden="true" style="display:none;">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="panel">
                    <div class="panel-heading">
                        <h3 class="panel-title">Update Certificate</h3>
...
<div class="modal" id="modal3" tabindex="-1" role="dialog" aria-hidden="true" style="display:none;">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="panel">
                    <div class="panel-heading">
                        <h3 class="panel-title">Remove Certificate</h3>
